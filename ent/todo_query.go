...
func (tq *TodoQuery) QueryAuthor() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, todo.AuthorTable, todo.AuthorColumn),
		)
...
func (tq *TodoQuery) WithAuthor(opts ...func(*UserQuery)) *TodoQuery {
	query := (&UserClient{config: tq.config}).Query()
...
func (tq *TodoQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Todo, error) {
	...
	if query := tq.withAuthor; query != nil {
		if err := tq.loadAuthor(ctx, query, nodes, nil,
			func(n *Todo, e *User) { n.Edges.Author = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TodoQuery) loadAuthor(ctx context.Context, query *UserQuery, nodes []*Todo, init func(*Todo), assign func(*Todo, *User)) error {
